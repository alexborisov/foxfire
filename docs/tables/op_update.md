...
Database columns to use during the insert query.
...
FOX_db::runUpdateQueryCol() functions like FOX_db::runUpdateQuery(), except the $col, $op, and $val parameters are passed in individually instead
of in an array. This means the function can only run a comparison against the contents of a single column.
/**
