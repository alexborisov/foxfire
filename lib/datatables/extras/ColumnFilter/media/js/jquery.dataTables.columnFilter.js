		    ...
		    if(this._oDataTable.fnSettings().oFeatures.bServerSide){
			    ...
			    this._oDataTable.fnSettings().fnServerData = function(sUrl, aoData, fnCallback, oSettings){
				    ...
				    for(j=0; j < parent._aiCustomSearch_Indexes.length; j++){
					    ...
					    for(k=0; k < aoData.length; k++){
						    ...
						    if( aoData[k].name == ("sSearch_" + index) ){
							    ...
							    var fromVal = $('#' + parent._oConfig.sTableId + 'range_from_' + index).val();
							    var toVal = $('#' + parent._oConfig.sTableId + 'range_to_' + index).val();
				    ...
				    if(fnServerDataOriginal != null){
					    ...
					    if(parent._oConfig.bTrace){ console.log({
										    method:'_fnRender',
										    action: 'Table has an existing remote data fetch function',
										    data: {
											    sUrl: sUrl,
											    aoData: aoData,
				    ...
				    // TODO: Is this necessary?
					    ...
					    if(parent._oConfig.bTrace){ console.log({
										    method:'_fnRender',
										    action: 'Table has no existing remote data fetch function',
										    data: {
											    sUrl: sUrl,
											    aoData: aoData
										    }
...
//						    }
//						
//						    oFunctionTimeout = window.setTimeout(
//						    
//							    function (){
...
//				    });				    
	    ...
	    */
	    "_fnCreateInput": function(args){
		    ...
		    // If we're *not using a remote data source*, use "search as you type" 
		    // to filter values
		    // ==============================================================================
		    ...
		    // Otherwise, filter on exact matches
		    else {			
			    input.keyup( function(){
		    ...
		    // When clicks in to an empty field, clear the hinting text
	    ...
	    */
	    "_fnCreateRangeInput": function(args){	    
		    ...
		    // Flag the column as requiring a complex index type
		    ...
		    // Replace the column's DOM filter function (which is used by dataTables.js when
		    // the table is not being populated via an AJAX call) with a custom filter
		    // function that can filter by range
		    // ==============================================================================
	    ...
	    */
	    "_fnCreateDateRangeInput": function(args){
		    ...
		    // Replace the column's DOM filter function (which is used by dataTables.js when
		    // the table is not being populated via an AJAX call) with a custom filter
		    // function that can filter by date ranges.
		    // ==============================================================================
