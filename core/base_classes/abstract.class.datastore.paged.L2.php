...
/**
 * FOXFIRE L2 PAGED ABSTRACT DATASTORE CLASS
 * Implements a highly efficient 2nd order paged datastore
 * 
 * FEATURES
 * --------------------------------------
 *  -> ACID compliant
...
abstract class FOX_dataStore_paged_L2_base extends FOX_db_base {
	...
	public function init($args=null){
		...
		$this->L2_col = $columns[0];
		$this->L1_col = $columns[1];	
		$this->L0_col = $columns[2];		
	...
	public function getL1( $L2, $L1s, $ctrl=null, &$valid=null){
	...
	public function getL2($L2s, $ctrl=null, &$valid=null){
	...
	/**
	 * Fetches multiple L2->L1 walks from the datastore
	 *
	 * @version 1.0
	 * @since 1.0
	 *
		...
		if($cache_fetch){
			...
			if($db_fetch){
				...
				if($db_result){
					...
					// Copy L2 pages in $this->cache to $update_cache as needed, then merge the db
					...
					foreach( $db_fetch as $L2 => $L1s ){		

						// If we're loading an entire L2 page from the db, we don't need to
...
//								unset($update_cache[$L3][$this->L2_col][$L2]);
							...
							// the L2 class cache page (if it exists) because the database
							// result doesn't have L2 authority

							if( FOX_sUtil::keyExists($L2, $this->cache) ){

								$update_cache[$L2] = $this->cache[$L2];				
							}
						}			    			    
						...
						foreach( $L1s as $L1 => $fake_var){
							...
							if( FOX_sUtil::keyExists($L1, $db_result[$L2]) ){

								$update_cache[$L2]["keys"][$L1] = $db_result[$L2][$L1];				
							}
						}
						unset($L1, $fake_var);
		...
		foreach( $get_data as $L2 => $L1s ){		
			...
			if( count($L1s) == 0 ){	    
				...
				if( FOX_sUtil::keyExists($L2, $cache_image) ){
				    
					$result[$L2] = $cache_image[$L2]['keys'];				
				}
				else {
					$valid = false;
				}
			...
			foreach( $L1s as $L1 => $fake_var){						    
				...
				if( FOX_sUtil::keyExists($L1, $cache_image[$L2]['keys'] ) ){

					$result[$L2][$L1] = $cache_image[$L2]['keys'][$L1];				
				}
				else {
					$valid = false;
				}							
	...
	public function notInClassCache($data, $cache_image){
		...
		foreach( $data as $L2 => $L1s ){				    
			...
			$L2_has_auth = FOX_sUtil::keyTrue('all_cached', $cache_image[$L2]);
			...
			if(!$L2_has_auth){  					
				...
				foreach( $L1s as $L1 => $val){
					...
					if( !FOX_sUtil::keyExists($L1, $cache_image[$L2]["keys"][$L1]) ){
	...
	public function addL1( $L2, $L1, $val, $ctrl=null){
	...
	public function addL2($L2, $L1s, $ctrl=null){
	...
	public function addL2_multi($data, $ctrl=null){
		...
		foreach( $data as $row ){
			...
			foreach( $row[$this->L1_col] as $L1 => $L1_val ){
				...
				$set_data[$row[$this->L2_col]][$L1] = $L1_val;
			}
			unset($L1, $L1_val);
	...
	public function addMulti($data, $ctrl=null){
		...
		if($ctrl['mode'] == 'matrix'){		   
				...
				try {			    
					...
					foreach( $data as $id => $row ){   			
						...
						$update_data[$row[$this->L2_col]][$row[$this->L1_col]] = $row[$this->L0_col];
		...
		foreach($update_cache as $L2 => $page_image){

			$this->cache[$L2] = $page_image;
		}
		unset($L2, $page_image);		
	...
	public function setL1($L2, $L1, $val, $ctrl=null){
	...
	public function setL2($L2, $L1s, $ctrl=null){
	...
	/**
	 * Creates or updates multiple L2->L1 walks which MAY OR MAY NOT ALREADY EXIST in the datastore
	 *
	 * @version 1.0
	 * @since 1.0
	 *
	...
	public function replaceL2( $L2, $data, $ctrl=null){
	...
	public function replaceL2_multi($data, $ctrl=null){
		...
		// Lock all L2 cache pages in the $data array
		// ===========================================================
		
		$L2_ids = array_keys($data);
...
//				unset($update_cache[$L2][$this->L2_col]);
		...
		// Clear all entries for the L2s from the db
		// ===========================================================

		$args = array(
				array("col"=>$this->L2_col, "op"=>"=", "val"=>$L2_ids)
		);
		...
		// Overwrite the locked L2 cache pages, releasing our lock
		// ===========================================================
		...
		// Flush any dead L2 cache pages, releasing our lock
		// ===========================================================
	...
	public function dropL1($L2, $L1s, $ctrl=null) {
	...
	/**
	 * Drops multiple [L2 + L1 branch] arrays from the datastore and cache
	 *
	 * @version 1.0
	 * @since 1.0
	 *
	...
	public function dropL2($L2s, $ctrl=null) {
	...
	/**
	 * Drops multiple [ L2 branch] arrays from the datastore and cache
	 *
	 * @version 1.0
	 * @since 1.0
	 *
	...
	/**
	 * Drops multiple L2->L1 walks from the datastore
	 *
	 * @version 1.0
	 * @since 1.0
	 *
	...
	public function dropMulti($data, $ctrl=null){
		...
		foreach( $del_data as $L2 => $L1s ){		
			...
			if( count($L1s) == 0 ){			// If the trie has no L4 structures, delete the
								// entire cache page from the class cache, and flag
				$dead_pages[] = $L2;		// the page to be flushed from the persistent cache			    
				unset($update_cache[$L2]);
			...
			$update_cache[$L2]['keys']	  = FOX_sUtil::arrayPrune($update_cache[$L2]['keys'], 2);			


			
			
			if( count($update_cache[$L2]['keys']) == 0 ){	    // If a cache page is empty after being pruned, delete 
...
} // End of class FOX_dataStore_paged_L2_base
