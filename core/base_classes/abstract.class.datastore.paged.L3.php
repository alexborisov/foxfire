...
abstract class FOX_dataStore_paged_L3_base extends FOX_db_base {
	...
	public function init($args=null){
		...
		$this->L3_col = $columns[0];
		$this->L2_col = $columns[1];
		$this->L1_col = $columns[2];	
		$this->L0_col = $columns[3];		
	...
	public function getL1($L3, $L2, $L1s, $ctrl=null, &$valid=null){
		...
		$get_data = array($L3=>array( $L2=>array() ));

		foreach( $L1s as $L1 ){
		    
			$get_data[$L3][$L2][$L1] = true;			
		}
		unset($L1);
	...
	public function getL2($L3, $L2s, $ctrl=null, &$valid=null){
	...
	public function getL3($L3s, $ctrl=null, &$valid=null){
	...
	/**
	 * Fetches multiple L3->L1 walks from the datastore
	 *
	 * @version 1.0
	 * @since 1.0
	 *
	...
	public function getMulti($data, $ctrl=null, &$valid=null){
		...
		if($cache_fetch){
			...
			if($db_fetch){
				...
				if($db_result){
					...
					// Copy L3 pages in $this->cache to $update_cache as needed, then merge the db
					// results into the L3 $update_cache pages. This saves memory over duplicating
					// the entire $this->cache variable.
					// ============================================================================				

					$update_cache = array();

					foreach( $db_fetch as $L3 => $L2s ){		

						// If we're loading an entire L3 page from the db, we don't need to
						...
						if( count($L2s) == 0 ){	    
							...
							if( FOX_sUtil::keyExists($L3, $db_result) ){
								...
								$update_cache[$L3]["keys"] = $db_result[$L3];				
							}
						}
						else {
							// However, for L2 or lower order objects, we have to merge into
							// the L3 class cache page (if it exists) because the database
							// result doesn't have L3 authority

							if( FOX_sUtil::keyExists($L3, $this->cache) ){

								$update_cache[$L3] = $this->cache[$L3];				
							}
						}			    			    
						...
						foreach( $L2s as $L2 => $L1s ){
							...
							if( count($L1s) == 0 ){	    
								...
								if( FOX_sUtil::keyExists($L2, $db_result[$L3]) ){

									// The L2 object now has authority
									$update_cache[$L3][$this->L2_col][$L2] = true;

									$update_cache[$L3]["keys"][$L2] = $db_result[$L3][$L2];				
								}
							}

							foreach( $L1s as $L1 => $fake_var){

								if( FOX_sUtil::keyExists($L1, $db_result[$L3][$L2]) ){

									$update_cache[$L3]["keys"][$L2][$L1] = $db_result[$L3][$L2][$L1];				
								}
							}
							unset($L1, $fake_var);
						}
						...
						if( FOX_sUtil::keyExists($L3, $db_result) ){
							...
							$update_cache[$L3][$this->L2_col] = FOX_sUtil::arrayPrune($update_cache[$L3][$this->L2_col], 3);								
						}
		...
		foreach( $get_data as $L3 => $L2s ){		
			...
			if( count($L2s) == 0 ){	    
				...
				if( FOX_sUtil::keyExists($L3, $cache_image) ){
				    
					$result[$L3] = $cache_image[$L3]['keys'];				
				}
				else {
					$valid = false;
				}
			...
			foreach( $L2s as $L2 => $L1s ){
				...
				if( count($L1s) == 0 ){	    
					...
					if( FOX_sUtil::keyExists($L2, $cache_image[$L3]['keys']) ){

						$result[$L3][$L2] = $cache_image[$L3]['keys'][$L2];				
					}
					else {
						$valid = false;
					}							
				...
				foreach( $L1s as $L1 => $fake_var){						    
					...
					if( FOX_sUtil::keyExists($L1, $cache_image[$L3]['keys'][$L2]) ){

						$result[$L3][$L2][$L1] = $cache_image[$L3]['keys'][$L2][$L1];				
					}
					else {
						$valid = false;
					}							
	...
	public function notInClassCache($data, $cache_image){
		...
		foreach( $data as $L3 => $L2s ){				    
			...
			$L3_has_auth = FOX_sUtil::keyTrue('all_cached', $cache_image[$L3]);
			...
			if(!$L3_has_auth){  					
				...
				foreach( $L2s as $L2 => $L1s ){
					...
					$L2_has_auth = FOX_sUtil::keyTrue($L2, $cache_image[$L3][$this->L2_col][$L2]);
	...
	public function addL1($L3, $L2, $L1, $val, $ctrl=null){
	...
	public function addL2($L3, $L2, $L1s, $ctrl=null){
	...
	public function addL2_multi($data, $ctrl=null){
		...
		foreach( $data as $row ){
			...
			foreach( $row[$this->L1_col] as $L1 => $L1_val ){
				...
				$set_data[$row[$this->L3_col]][$row[$this->L2_col]][$L1] = $L1_val;
			}
			unset($L1, $L1_val);
	...
	public function addL3($L3, $L2s, $ctrl=null){
		...
		foreach( $data as $row ){
			...
			foreach( $row[$this->L2_col] as $L2 => $L1s ){
				...
				foreach( $L1s as $L1 => $L1_val ){
					...
					$set_data[$row[$this->L3_col]][$L2][$L1] = $L1_val;
				}
				unset($L1, $L1_val);
			}
			unset($L2, $L1s);			
	...
	/**
	 * Creates multiple L3->L1 walks. This method is used when adding walks that DO NOT ALREADY EXIST
	...
	public function addMulti($data, $ctrl=null){
		...
		if($ctrl['mode'] == 'matrix'){		   
				...
				try {			    
					...
					foreach( $data as $id => $row ){   			
						...
						$update_data[$row[$this->L3_col]][$row[$this->L2_col]][$row[$this->L1_col]] = $row[$this->L0_col];
		...
		foreach($update_cache as $L3 => $page_image){
	...
	public function setL1($L3, $L2, $L1, $val, $ctrl=null){
	...
	public function setL2($L3, $L2, $L1s, $ctrl=null){
	...
	public function setL3($L3, $L2s, $ctrl=null){
	...
	public function replaceL2($L3, $L2, $data, $ctrl=null){
		...
		$replace_data = array( $L3=>array( $L2=>$data ));
	 ...
	 * deleting all L2->L1 walks for each L3->L2 intersect structure passed in the $data array, 
	...
	public function replaceL3($L3, $data, $ctrl=null){
	...
	public function dropL1($L3, $L2, $L1s, $ctrl=null) {
	...
	/**
	 * Drops multiple [L3->L2 walk + L1 branch] arrays from the datastore and cache
	 *
	 * @version 1.0
	 * @since 1.0
	 *
	...
	/**
	 * Drops one or more L2 branches within a single L3 from the datastore and cache
	 *
	 * @version 1.0
	 * @since 1.0
	 *
	...
	public function dropL2($L3, $L2s, $ctrl=null) {
	...
	/**
	 * Drops multiple [L3 => L2 branch] arrays from the datastore and cache
	 *
	 * @version 1.0
	 * @since 1.0
	 *
	...
	public function dropL3($L3s, $ctrl=null) {
	...
	/**
	 * Drops multiple [L3 branch] arrays from the datastore and cache
	 *
	 * @version 1.0
	 * @since 1.0
	 *
	...
	/**
	 * Drops multiple L3->L1 walks from the datastore
	 *
	 * @version 1.0
	 * @since 1.0
	 *
	...
	public function dropMulti($data, $ctrl=null){
		...
		foreach( $del_data as $L3 => $L2s ){		
			...
			if( count($L2s) == 0 ){			// If the trie has no L4 structures, delete the
								// entire cache page from the class cache, and flag
				$dead_pages[] = $L3;		// the page to be flushed from the persistent cache			    
			...
			foreach( $L2s as $L2 => $L1s ){
				...
				if( count($L1s) == 0 ){	    // If the L2 structure has no L1 structures, 
					...
					unset($update_cache[$L3][$this->L2_col][$L2]);
					unset($update_cache[$L3]["keys"][$L2]);
				}

				foreach( $L1s as $L1 => $val){

					unset($update_cache[$L3]["keys"][$L2][$L1]);
				}
				unset($L1, $val);
			}
			unset($L2, $L1s);
			...
			$update_cache[$L3]['keys']	  = FOX_sUtil::arrayPrune($update_cache[$L3]['keys'], 3);			
			$update_cache[$L3][$this->L2_col] = FOX_sUtil::arrayPrune($update_cache[$L3][$this->L2_col], 2);


			
			
			if( count($update_cache[$L3]['keys']) == 0 ){	    // If a cache page is empty after being pruned, delete 
	...
	public function dropGlobal($level, $items, $ctrl=null) {
		...
		// Since this operation affects ALL L3 pages, we have to flush the 
