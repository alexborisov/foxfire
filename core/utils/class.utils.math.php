...
class FOX_math {
	...
	/**
	 * Encodes a base-10 formatted string to an arbitrary base
	 *
		...
		if( preg_match('/^[' . $chars . ']+$/', $str) ){
			...
			for ($i=0; $i<strlen($str); $i++){
				...
				if( $i != 0 ){
					...
					$result = bcmul($result, strlen($chars));
				}
	...
	/**
	 * Converts a string encoded in an arbitrary base to base-10 format
	 *
