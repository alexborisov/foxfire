		...
		foreach( $args as $col_params ){
			...
			if( !is_array($col_params["val"]) ){
				...
				switch($col_params["op"]){
					...
					case "<>" :
					{
						$where .= $prefix . $col_params["col"] . " " . $col_params["op"];
					...
					case "!LIKE" :
					{
						...
						$where .= $prefix . $col_params["col"] . " " . $op;
						$where .= " " . $struct["columns"][$col_params["col"]]["format"];
					...
					case "MATCH" :
					{					    
						$where .= "MATCH(" . $prefix . $col_params["col"] . ") ";
						$where .= "AGAINST(" . $struct["columns"][$col_params["col"]]["format"] . ")";					    
			...
			// ======================================================================================
				...
				elseif( $col_params["op"] == "MATCH" ) {

					$where .= "MATCH(" . $prefix . $col_params["col"] . ") AGAINST(";
				}				
				elseif( ($col_params["op"] == "LIKE") || ($col_params["op"] == "!LIKE")) {

					// LIKE and NOT LIKE statements have to be structured as [LIKE "%thing1" AND LIKE "%thing2"],
					// the list form [LIKE("%thing1", "%thing2")] is not valid syntax
				}				
				else {

					$text =  "\nMethod {$caller}() called buildWhere() with an illegal SQL comparison operator for a SQL IN() construct.";
					$text .= "\nValid operators for this query type are: '=', '!=', '<>', 'LIKE', '!LIKE'.\n";
					$text .= "\nColumn name: '{$col_params["col"]}' \nBad Operator: '{$col_params["op"]} '\n";
				...
				switch($col_params["op"]){
				    ...
				    case '!LIKE' : {
					    ...
					    // The statements must be grouped inside of brackets so they're evaluated as
					    // a single construct, otherwise it will break AND-OR chains
					    // [IN("a","b") OR] vs [LIKE "%a" AND LIKE "%b" OR] vs [(LIKE "%a" AND LIKE "%b") OR]
		...
		if( is_array($data) ){
			...
			foreach($data as $block){
				...
				foreach($column_names as $column_name){
					...
					if(FOX_sUtil::keyExists($column_name, $block)){
					    
						$param_val = $block[$column_name];					    
					}
					else {
						$param_val = null;
					}
					
					$params_list[] = array('escape'=>$escape, 'val'=>$cast->PHPToSQL($param_val, $in_type, $out_type) );
