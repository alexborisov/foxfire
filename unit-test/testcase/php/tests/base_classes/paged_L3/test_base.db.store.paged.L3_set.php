...
class FOX_dataStore_paged_L3_tester_setMethods extends FOX_dataStore_paged_L3_base {
...
}  // ENDOF: class FOX_dataStore_paged_L3_tester_setMethods
  
                                      

/**
 * FOXFIRE UNIT TEST SCRIPT - L3 PAGED ABSTRACT DATASTORE CLASS - SET METHODS
 *
 * @version 1.0
 * @since 1.0
 * @package FoxFire
...
class core_L3_paged_abstract_setMethods extends RAZ_testCase {
		...
		$this->cls = new FOX_dataStore_paged_L3_tester_setMethods();
	...
	*/	
	public function test_setL1() {
		...
		foreach( $test_data as $item ){
			...
			try {
				$set_ok = $this->cls->setL1($item['L3'], $item['L2'], $item['L1'], $item['L0'], $ctrl=null);
			}
			catch (FOX_exception $child) {
		...
		try {
			$rows_changed = $this->cls->setL1(1, 'X', 3, 0, $ctrl=null);
	...
	*/	
	public function test_setL2() {
		...
		$test_data = array(
				...
				array( "L3"=>1, "L2"=>"X", "L1s"=>array(
										    1=>null,
										    2=>false,
										    5=>true,
										    3=>(int)0
										)), 
				array( "L3"=>1, "L2"=>"Y", "L1s"=>array(
										    1=>(int)1,
										    2=>(int)-1,
										    3=>(float)1.7,
										    4=>(float)-1.6
										)), 
		    		array( "L3"=>2, "L2"=>"K", "L1s"=>array(
										    1=>(string)"foo",
										    2=>array(null, true, false, 1, 1.0, "foo"),
										    3=>$test_obj
										))
		...
		foreach( $test_data as $item ){
			...
			try {
				$rows_changed = $this->cls->setL2($item['L3'], $item['L2'], $item['L1s'], $ctrl=null);
			}
			catch (FOX_exception $child) {
			...
			$this->assertEquals(count($item['L1s']), $rows_changed, ("ITEM: " . $item['L3'] . $item['L2'])); 			
		...
		try {		    
			...
			$rows_changed = $this->cls->setL2(1, "X", array( 3=>(int)0 ), $ctrl=null);
	...
	*/	
	function test_setL2_multi() {
		...
		$test_data = array(
				...
				array( "L3"=>1, "L2"=>"X", "L1"=>array(
										    1=>null,
										    2=>false,
										    5=>true,
										    3=>(int)0
										)), 
				array( "L3"=>1, "L2"=>"Y", "L1"=>array(
										    1=>(int)1,
										    2=>(int)-1,
										    3=>(float)1.7,
										    4=>(float)-1.6
										)), 
		    		array( "L3"=>2, "L2"=>"K", "L1"=>array(
										    1=>(string)"foo",
										    2=>array(null, true, false, 1, 1.0, "foo"),
										    3=>$test_obj
										))
		...
		$dupe_items = array(
				array( "L3"=>1, "L2"=>"X", "L1"=>array(1=>null,2=>false))
		);
		...
		$dupe_items = array(
				array( "L3"=>1, "L2"=>"X", "L1"=>array(3=>(int)0)),
				array( "L3"=>1, "L2"=>"Y", "L1"=>array(1=>(int)1,2=>(int)-1)),
		);
	...
	*/	
	...
	function test_setL3() {
		...
		foreach( $test_data as $item ){
			...
			try {
				$rows_changed = $this->cls->setL3($item['L3'], $item['L2s'], $ctrl=null);
			}
			catch (FOX_exception $child) {
			...
			$this->assertEquals( $item['count'], $rows_changed, ("ITEM: " . $item['L3']) ); 
		...
		try {			
			$rows_changed = $this->cls->setL3(1, array( "Y"=>array(
										    1=>(int)1,												
										    2=>(int)-1,									
										    3=>(float)1.7,
										    4=>(float)-1.6
