...
class FOX_dataStore_paged_L3_tester_globalMethods extends FOX_dataStore_paged_L3_base {
...
}  // ENDOF: class FOX_dataStore_paged_L3_tester_globalMethods 

                                      

/**
 * FOXFIRE UNIT TEST SCRIPT - L3 PAGED ABSTRACT DATASTORE CLASS - GLOBAL METHODS
 *
 * @version 1.0
 * @since 1.0
 * @package FoxFire
...
class core_L3_paged_abstract_globalMethods extends RAZ_testCase {
		...
		$this->cls = new FOX_dataStore_paged_L3_tester_globalMethods();
	...
	*/	
	public function test_dropGlobal_L2_single() {
		...
		try {			
			$rows_changed = $this->cls->dropGlobal(2, 'X', $drop_ctrl);
		}
		catch (FOX_exception $child) {
		...
		$this->assertEquals(false, $valid);  // Should report false because L3 2
						    // does not exist
	...
	*/	
	public function test_dropGlobal_L2_multi() {
		...
		try {			
			$rows_changed = $this->cls->dropGlobal(2, array('X','Y'), $drop_ctrl);
		}
		catch (FOX_exception $child) {
