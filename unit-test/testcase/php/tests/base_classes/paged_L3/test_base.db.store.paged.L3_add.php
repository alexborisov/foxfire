...
class FOX_dataStore_paged_L3_tester_addMethods extends FOX_dataStore_paged_L3_base {
...
}  // ENDOF: class FOX_dataStore_paged_L3_tester_addMethods
  
                                      

/**
 * FOXFIRE UNIT TEST SCRIPT - L3 PAGED ABSTRACT DATASTORE CLASS - ADD METHODS
 *
 * @version 1.0
 * @since 1.0
 * @package FoxFire
...
class core_L3_paged_abstract_addMethods extends RAZ_testCase {
		...
		$this->cls = new FOX_dataStore_paged_L3_tester_addMethods();
	...
	*/	
	public function test_addL1() {
		...
		foreach( $test_data as $item ){
			...
			try {
				$set_ok = $this->cls->addL1( $item['L3'], $item['L2'], $item['L1'], $item['L0'], $ctrl=null);
			}
			catch (FOX_exception $child) {
		...
		try {
			$this->cls->addL1(1, 'X', 3, 0, $ctrl=null);
	...
	*/	
	public function test_addL2() {
		...
		foreach( $test_data as $item ){
			...
			try {
				$rows_changed = $this->cls->addL2($item['L3'], $item['L2'], $item['L1s'], $ctrl=null);
			}
			catch (FOX_exception $child) {
		...
		try {		    
			...
			$this->cls->addL2(1, "X", array( 3=>(int)0 ), $ctrl=null);
	...
	*/	
	...
	function test_addL3() {
		...
		foreach( $test_data as $item ){
			...
			try {
				$rows_changed = $this->cls->addL3( $item['L3'], $item['L2s'], $ctrl=null);
			}
			catch (FOX_exception $child) {
